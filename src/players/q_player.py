...
from ..models import QLearningAgent, NewQAgent
...
class QLearningPlayer():
    def __init__(self) -> None:
        ...
        self.env: gymnasium.Env = BaseNumberGuessingEnv()
        self.done = False
        self.obs = None
        self.lastGuess = None
    ...
    def train(self, totalEpisodes: int = 1000) -> gymnasium.Env:
        ...
        for episode in range(totalEpisodes):
            ...
            while not self.done:
                self.lastGuess = action = self.agent.chooseAction(self.obs)
                nextState, reward, self.done, info = self.env.step(action)
                self.agent.learn(self.obs, action, reward, nextState, self.done)
                self.obs = nextState
            ...
            if episode % (totalEpisodes * 0.05) == 0:
    ...
    def play(self) -> None:
        ...
        while not self.done:
            ...
            action = self.agent.chooseAction(self.obs)
